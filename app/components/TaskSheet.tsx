...
export function TaskSheet({ items, openTaskId, onTaskOpened, onCreate }: Props) {
  ...
  return <>
    ...
    <div className="task-sheet-wrap" tabIndex={0} aria-label="Bảng tính công việc">
      <table className="task-sheet"><thead><tr><th className="sheet-checkbox"><input type="checkbox" aria-label="Chọn tất cả dòng" checked={Boolean(items.length && selectedIds.length === items.length)} onChange={toggleAll}/></th><th>#</th><th>Mã</th><th>Công việc</th><th>Người phụ trách</th><th>Loại</th><th>Thời gian</th><th>Giờ BĐ</th><th>Giờ KT</th><th>Định dạng</th><th>Trạng thái</th><th aria-label="Thao tác"/></tr></thead><tbody>{items.length ? items.map((task, index) => {
        const startDate = taskStartDate(task) ?? task.deadline ?? "";
        const isSaving = savingIds.has(task.id);
        return <tr className={selectedIds.includes(task.id) ? "selected" : ""} key={task.id}><td className="sheet-checkbox"><input type="checkbox" aria-label={`Chọn ${task.title}`} checked={selectedIds.includes(task.id)} onChange={() => toggle(task.id)}/></td><td className="sheet-row-number">{index + 1}</td><td className="sheet-code"><button type="button" onClick={() => setSelected(task)} aria-label={`Mở chi tiết và chỉnh sửa ${task.code}`}>{task.code}</button></td><td><SheetTextCell value={task.title} required ariaLabel={`Tên công việc ${task.code}`} onSave={(title) => save(task, { title })}/></td><td><button type="button" className="sheet-assignees" onClick={() => setSelected(task)} aria-label={`Chỉnh người phụ trách ${task.title}`}>{task.owner}</button></td><td><select className="sheet-select" value={task.workType} disabled={isSaving} aria-label={`Loại công việc ${task.title}`} onChange={(event) => void save(task, { workType: event.target.value as TaskInput["workType"] })}>{WORK_TYPES.map((type) => <option value={type} key={type}>{workTypeLabels[type]}</option>)}</select></td><td><button type="button" className="sheet-range-trigger" disabled={isSaving} onClick={() => setDateTask(task)} aria-label={`Chọn khoảng thời gian ${task.title}`}>{startDate && task.deadline ? formatRange(startDate, task.deadline) : "Chọn khoảng ngày"}<span aria-hidden="true">⌄</span></button></td><td><select className="sheet-select" value={task.startTime} disabled={isSaving} aria-label={`Giờ bắt đầu ${task.title}`} onChange={(event) => { const startTime = event.target.value; const endTime = task.endTime > startTime ? task.endTime : timeOptions[timeOptions.indexOf(startTime) + 1]; void save(task, { startTime, endTime }); }}>{timeOptions.slice(0, -1).map((time) => <option value={time} key={time}>{time}</option>)}</select></td><td><select className="sheet-select" value={task.endTime} disabled={isSaving} aria-label={`Giờ kết thúc ${task.title}`} onChange={(event) => void save(task, { endTime: event.target.value })}>{timeOptions.filter((time) => time > task.startTime).map((time) => <option value={time} key={time}>{time}</option>)}</select></td><td><SheetTextCell value={task.format} ariaLabel={`Định dạng ${task.title}`} onSave={(format) => save(task, { format })}/></td><td><select className={`sheet-select sheet-status ${task.status}`} value={task.status} disabled={isSaving} aria-label={`Trạng thái ${task.title}`} onChange={(event) => void save(task, { status: event.target.value as TaskStatus })}>{TASK_STATUSES.map((status) => <option value={status} key={status}>{statusLabels[status]}</option>)}</select></td><td><button className="sheet-open-task" type="button" onClick={() => setSelected(task)} aria-label={`Mở chi tiết ${task.title}`}>Mở</button></td></tr>;
      }) : <tr><td className="sheet-empty" colSpan={12}>Không có công việc phù hợp. Hãy thay đổi bộ lọc hoặc thêm dòng mới.</td></tr>}</tbody></table>
    </div>
