...
function newTask(): TaskInput { const today = todayIso(); return { title: "", owner: "Chưa phân công", assigneeIds: [], workType: "inhouse", status: "todo", startDate: today, deadline: today, startTime: "09:00", endTime: "11:00", reminderDate: null, reminderTime: null, reminderRepeat: "none", reminderOffsets: [], format: "", brief: "" }; }
...
export function TaskFormModal({ task, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    setForm(task ? { title: task.title, owner: task.owner, assigneeIds: task.assigneeIds, workType: task.workType, status: task.status, startDate: task.startDate ?? task.deadline ?? todayIso(), deadline: task.deadline ?? task.startDate ?? todayIso(), startTime: task.startTime, endTime: task.endTime ?? "11:00", reminderDate: task.reminderDate, reminderTime: task.reminderTime, reminderRepeat: task.reminderRepeat, reminderOffsets: task.reminderOffsets, format: task.format, brief: task.brief } : newTask());
    setError(null);
  }, [task]);
