...
export function DateRangePicker({ startDate, endDate, onChange, onComplete, label = "Từ ngày – đến ngày", openOnMount = false, singleDate = false }: Props) {
  ...
  return <div className="date-range-picker" ref={root}>
    ...
    {open && <div className="date-range-popover" role="dialog" aria-label={label}>
      ...
      <div className="date-range-footer"><button type="button" onClick={() => { const today = toIso(new Date()); onChange(today, today); setMonth(monthStart(today)); setSelectingEnd(!singleDate); if (singleDate) { onComplete?.(today, today); setOpen(false); } }}>Hôm nay</button><button type="button" onClick={() => { if (startDate && endDate) onComplete?.(startDate, endDate); setSelectingEnd(false); setHoveredDate(null); setOpen(false); }}>Xong</button></div>
    </div>}
  </div>;
}
