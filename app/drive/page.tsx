...
export default function DrivePage() {
  ...
  const load = useCallback(async (folderId?: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/api/drive${folderId ? `?folderId=${encodeURIComponent(folderId)}` : ""}`, { cache: "no-store" });
      const body = await response.json() as DriveResponse;
      if (!response.ok) throw new Error(body.error ?? "Không thể tải Google Drive.");
      setConnected(body.connected);
      setItems(body.items);
      setError(null);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Không thể tải Google Drive.");
  ...
  const request = async (url: string, init?: RequestInit) => {
    ...
    if (!response.ok) throw new Error(body.error ?? "Yêu cầu Google Drive không thành công.");
    return body;
  };
  ...
  const upload = async (file?: File) => {
    ...
    finally { setWorking(false); if (uploadInput.current) uploadInput.current.value = ""; }
  };

  const openFolder = (item: DriveItem) => { setTrail((current) => [...current, { id: item.id, name: item.name }]); setSelectedVideo(null); void load(item.id); };
  const returnTo = (index: number) => { const next = trail.slice(0, index + 1); setTrail(next); setSelectedVideo(null); void load(next.at(-1)?.id); };
  const videos = items.filter((item) => item.kind === "video");

  return <WorkspaceShell title="Video & Drive"><div className="drive-page"><div className="drive-heading"><div><small>MEDIA LIBRARY / GOOGLE DRIVE</small><h1>Video & Drive</h1><p>Video, tài liệu và thư mục được đồng bộ trực tiếp với Google Drive.</p></div>{connected && isAdmin && <div className="drive-actions"><input ref={uploadInput} className="visually-hidden" type="file" onChange={(event) => void upload(event.target.files?.[0])}/><button className="secondary icon-button" onClick={() => void load(currentFolderId)} disabled={loading || working} title="Đồng bộ lại"><Icon name="refresh"/></button><button className="secondary" onClick={() => setFolderDialog(true)}><Icon name="folder" size={16}/> Thư mục</button><button className="primary" onClick={() => uploadInput.current?.click()} disabled={working}><Icon name="upload" size={16}/> Upload</button></div>}</div>{error && <div className="drive-alert"><Icon name="warning" size={18}/><span>{error}</span><button className="text-button" onClick={() => void load(currentFolderId)}>Thử lại</button></div>}{connected === false ? <ConnectDriveCard/> : <><div className="drive-toolbar"><div className="breadcrumbs"><button onClick={() => { setTrail([]); setSelectedVideo(null); void load(); }}>Drive</button>{trail.map((item, index) => <span key={item.id}><b>/</b><button onClick={() => returnTo(index)}>{item.name}</button></span>)}</div><span>{loading ? "Đang đồng bộ…" : `${items.length} mục`}</span></div>{selectedVideo && <VideoPlayer item={selectedVideo} onClose={() => setSelectedVideo(null)}/>}<div className="drive-grid">{loading ? <div className="drive-empty">Đang tải dữ liệu Drive…</div> : items.length === 0 ? <div className="drive-empty"><Icon name="folder" size={30}/><b>Thư mục này đang trống</b><span>{isAdmin ? "Tạo thư mục hoặc upload file để bắt đầu." : "Chưa có video hoặc tài liệu nào."}</span></div> : items.map((item) => <DriveCard key={item.id} item={item} isAdmin={isAdmin} onOpenFolder={() => openFolder(item)} onPlay={() => setSelectedVideo(item)} onRename={() => setRenameItem(item)} onDelete={() => void remove(item)} />)}</div>{videos.length > 0 && !selectedVideo && <p className="drive-video-hint">Chọn một video để xem trực tiếp trong workspace.</p>}</>}</div>{folderDialog && <NameDialog title="Tạo thư mục mới" label="Tên thư mục" submitLabel="Tạo thư mục" submitting={working} onClose={() => setFolderDialog(false)} onSubmit={createFolder}/>} {renameItem && <NameDialog title="Đổi tên" label={renameItem.kind === "folder" ? "Tên thư mục" : "Tên tệp"} initialValue={renameItem.name} submitLabel="Lưu thay đổi" submitting={working} onClose={() => setRenameItem(null)} onSubmit={rename}/>}</WorkspaceShell>;
}
