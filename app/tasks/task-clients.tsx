...
export default function TasksClient() {
  ...
  return <WorkspaceShell title="Tất cả công việc"><WorkspaceState><div className="page-heading"><div><small>WORKSPACE / TASKS</small><h1>Tất cả công việc</h1><p>Quản lý toàn bộ nội dung, deadline và người phụ trách.</p></div><div className="page-heading-actions"><Link className="secondary" href="/calendar">Xem lịch</Link><button className="primary" onClick={() => setCreating(true)}>＋ Tạo công việc</button></div></div><div className="task-toolbar"><div className="tabs">{(["all", "inhouse", "outsource"] as const).map((item) => <button key={item} className={type === item ? "tab active" : "tab"} onClick={() => setType(item)}>{item === "all" ? `Tất cả (${tasks.length})` : `${workTypeLabels[item]} (${tasks.filter((task) => task.workType === item).length})`}</button>)}</div><div className="task-view-toggle" aria-label="Chế độ hiển thị công việc"><button type="button" className={view === "kanban" ? "active" : ""} onClick={() => setView("kanban")}>Kanban</button><button type="button" className={view === "table" ? "active" : ""} onClick={() => setView("table")}>Danh sách</button></div><label className="search-field"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm task, người phụ trách..." /></label></div><div className="filters"><label>Trạng thái<select value={status} onChange={(event) => setStatus(event.target.value as "all" | TaskStatus)}><option value="all">Tất cả trạng thái</option>{Object.entries(statusLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label>Người phụ trách<select value={owner} onChange={(event) => setOwner(event.target.value)}><option value="all">Tất cả thành viên</option>{members.map((member) => <option key={member.id} value={member.name}>{member.name}</option>)}</select></label><button className="secondary" onClick={() => { setType("all"); setStatus("all"); setOwner("all"); setQuery(""); }}>Xoá bộ lọc</button></div>{view === "kanban" ? <TaskKanban items={visible} openTaskId={searchParams.get("task")} onTaskOpened={() => router.replace("/tasks", { scroll: false })}/> : <TaskTable items={visible} openTaskId={searchParams.get("task")} onTaskOpened={() => router.replace("/tasks", { scroll: false })}/>}</WorkspaceState>{creating && <TaskFormModal onClose={() => setCreating(false)} />}</WorkspaceShell>;
}
